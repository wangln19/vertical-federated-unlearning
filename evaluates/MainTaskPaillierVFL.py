...
class MainTaskPaillierVFL(object):
    ...
    def train(self):
        ...
        for i_epoch in range(self.epochs):
            ...
            for parties_data in zip(*data_loader_list):
                self.gt_one_hot_label = self.label_to_one_hot(
            ...
            if (i + 1) % print_every == 0:
                ...
                with torch.no_grad():
                    data_loader_list = [
                        self.parties[ik].test_loader for ik in range(self.k)
                    ]
                    for parties_data in zip(*data_loader_list):
                        ...
                        for ik in range(self.k):
                            ...
                            # ####### Noisy Sample #########
                                ...
                                pred_list.append(
                                    self.parties[ik].local_model(
                                        noisy_sample(parties_data[ik][0], scale)
                                    )
                                )
                            # ####### Noisy Sample #########
                            else:
                                pred_list.append(
                                    self.parties[ik].local_model(parties_data[ik][0])
                                )
    ...
    def save_state(self, BEFORE_MODEL_UPDATE=True):
        if BEFORE_MODEL_UPDATE:
            return {
                "model": [
                    copy.deepcopy(self.parties[ik].local_model)
            ...
            return {
                ...
                "predict": [
                    copy.deepcopy(self.parties[ik].local_pred_clone)
                    for ik in range(self.k)
                ],
                "gradient": [
                ...
                "final_global_model": copy.deepcopy(
                    self.parties[self.args.k - 1].global_model
                ),
            }
    ...
    def save_party_data(self):
        return {
            "aux_data": [
                copy.deepcopy(self.parties[ik].aux_data) for ik in range(self.k)
            ],
            "train_data": [
                copy.deepcopy(self.parties[ik].train_data) for ik in range(self.k)
            ],
            "test_data": [
                copy.deepcopy(self.parties[ik].test_data) for ik in range(self.k)
            ],
            "aux_label": [
                copy.deepcopy(self.parties[ik].aux_label) for ik in range(self.k)
            ],
            "train_label": [
                copy.deepcopy(self.parties[ik].train_label) for ik in range(self.k)
            ],
            "test_label": [
                copy.deepcopy(self.parties[ik].test_label) for ik in range(self.k)
            ],
            "aux_loader": [
                copy.deepcopy(self.parties[ik].aux_loader) for ik in range(self.k)
            ],
            "train_loader": [
                copy.deepcopy(self.parties[ik].train_loader) for ik in range(self.k)
            ],
            "test_loader": [
                copy.deepcopy(self.parties[ik].test_loader) for ik in range(self.k)
            ],
    ...
    def save_trained_models(self):
        ...
        torch.save(
            (
                [
                    self.trained_models["model"][i].state_dict()
    ...
    def calc_label_recovery_rate(self, dummy_label, gt_label):
        success = torch.sum(
            torch.argmax(dummy_label, dim=-1) == torch.argmax(gt_label, dim=-1)
        ).item()
