...
def evaluate_no_attack(args):
    ...
    vfl.determine_save_dir()
    append_exp_res(args.exp_res_path, vfl.dir_path)
    if not args.trained:
        if args.dataset not in ['cora']:
            total_time, communication_cost, final_epoch = vfl.train()
        ...
        vfl.load_trained_model(vfl.dir_path + 'Trained_model.pkl')
        distance, test_loss, test_acc, test_auc = vfl.fidelity_evaluation()
        ...
        gradient_residue_sum, gradient_residue_list = vfl.efficacy_evaluation()
        ...
        if args.feature_inference_list != []:
            if args.remove_specific_information:
                args.attack_specific_information = True
                args.attack_features = False
                args.remove_specific_information = False
                for ik in range(args.k):
                    args.parties[ik].prepare_data(args, ik)
                vfl.final_state.update(vfl.save_party_data())
            elif args.remove_specific_features:
                args.attack_features = True
                args.attack_specific_information = False
                args.remove_specific_features = False
                for ik in range(args.k):
                    args.parties[ik].prepare_data(args, ik)
                vfl.final_state.update(vfl.save_party_data())
            else:
            ...
            for index in args.feature_inference_index:
                ...
                append_exp_res(args.exp_res_path, f"==unlearning_features==")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_distribute_percent', type=bool, default=True, help='whether to save the distribute percent')
    parser.add_argument('--save_overall_pred', type=bool, default=True, help='whether to save the overall prediction')
    parser.add_argument('--remove_specific_features', type=bool, default=False, help='specific features to be removed')
    parser.add_argument('--random_remove_features_percentage', type=float, default=0.0, help='random remove features percentage')
    ...
    parser.add_argument('--regular_h', type=float, default=0.0, help='hyper to regular z')
    ...
    for seed in [97]: # test 5 times 
        ...
        filename = f'training_record.txt'
        args.exp_res_path = args.exp_res_dir + filename
        print(args.exp_res_path)
        print('=============unlearning set====================')
        print('remove specific features :',args.remove_specific_features)
        print('random remove features percentage :',args.random_remove_features_percentage)
        print('remove specific samples :',args.remove_specific_samples)
        print('random remove samples percentage :',args.random_remove_samples_percentage)
        print('remove specific information :',args.remove_specific_information)
        print('random remove information percentage :',args.random_remove_information_percentage)
        print('remove specific clients :',args.remove_specific_clients)
        print('=================================\n')
        ...
        if args.remove_specific_clients or args.remove_specific_features:
