...
class NUSWIDEDatasetVFL():
    ...
    def __init__(self, data_dir, data_type, target_number=10, target_label=0):
        ...
        poison_idx = need_poison_down_check_nuswide_vfl(np.array(X_text).astype('float32'))
...
def need_poison_down_check_nuswide_vfl(images):
...
if __name__ == '__main__':
    ...
    train_dataset = NUSWIDEDatasetVFL(data_dir, 'train')
    print(train_dataset.y)

    # print(train_dataset.poison_list)

    res = need_poison_down_check_nuswide_vfl(train_dataset.x[1])
    print(res.sum())
