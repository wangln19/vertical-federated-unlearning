...
from evaluates.UnlearningTask import *
...
def unlearning(args):
    ...
    vfu = UnlearningTask(args)
    vfu.determine_save_dir()
    append_exp_res(args.exp_res_path, vfu.dir_path)

    if not args.trained:
        if args.stream_unlearning:
            vfu.load4stream_unlearning()
        if args.regular_h:
            total_time, communication_cost, final_epoch = vfu.baseline_VFULR()
        else:
            total_time, communication_cost, final_epoch = vfu.unlearning_retrain_based_on_h()
        efficiency_result = f"Efficiency: total_time, communication_cost, final_epoch = {total_time}, {communication_cost}, {final_epoch}\n"
        print(efficiency_result)
        append_exp_res(args.exp_res_path, efficiency_result)
        if args.stream_unlearning:
            vfu.save4stream_unlearning()
        return 
    else:
        if not args.wth_asynchronous_unlearning:
            vfu.load_trained_model(vfu.dir_path + 'Unlearning_model.pkl')
        else:
            vfu.load_trained_model(vfu.dir_path + 'Unlearning_model.pkl')
            vfu.load_trained_model(vfu.dir_path.replace('Asynchronous_Unlearning','Asynchronous_Unlearning1') + 'Unlearning_model.pkl')
        distance, test_loss, test_acc, test_auc = vfu.fidelity_evaluation()
        fidelity_result = f"Fidelity: distance, test_loss, test_acc, test_auc = {distance}, {test_loss}, {test_acc}, {test_auc}\n"
        print(fidelity_result)
        append_exp_res(args.exp_res_path, fidelity_result)
        gradient_residue_sum, gradient_residue_list = vfu.efficacy_evaluation()
        efficacy_result = f"Efficacy: gradient_residue_sum, gradient_residue_list = {gradient_residue_sum}, {gradient_residue_list}\n"
        print(efficacy_result)
        append_exp_res(args.exp_res_path, efficacy_result)

        if args.feature_inference_list != [] and (args.unlearning_specific_information or args.unlearning_features or args.unlearning_clients):
            if args.unlearning_specific_information:
                args.attack_specific_information = True
                args.attack_features = False
            elif args.unlearning_features or args.unlearning_clients:
                args.attack_specific_information = False
                args.attack_features = True
            else:
                args.attack_specific_information = False
                args.attack_features = False
            for index in args.feature_inference_index:
                ...
                append_exp_res(args.exp_res_path, f"==unlearning_specific_information==")
                rand_mse, mse = vfu.evaluate_attack()
                FIA_result = f"FIA mse_reduction: rand_mse, mse = {rand_mse}, {mse}"
                print(FIA_result)
                append_exp_res(args.exp_res_path, FIA_result)
    
    # if args.dataset not in ['cora']:
        
    #     main_acc , stopping_iter, stopping_time, stopping_commu_cost= vfu.train()
    # else:
    #     main_acc, stopping_iter, stopping_time = vfu.train_graph()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--configs', type=str, default='test_unlearning', help='configure json file path')
    parser.add_argument('--save_model', type=bool, default=True, help='whether to save the trained model')
    parser.add_argument('--save_distribute_percent', type=bool, default=False, help='whether to save the distribute percent')
    parser.add_argument('--save_overall_pred', type=bool, default=False, help='whether to save the overall prediction')
    parser.add_argument('--save_h_history', type=bool, default=False, help='whether to save the h history')
    ############ Do not change ############
    parser.add_argument('--remove_specific_features', type=bool, default=False, help='specific features to be removed')
    parser.add_argument('--random_remove_features_percentage', type=float, default=0, help='random remove features percentage')
    parser.add_argument('--remove_specific_samples', type=bool, default=False, help='specific samples to be removed')
    parser.add_argument('--random_remove_samples_percentage', type=float, default=0.0, help='random remove samples percentage')
    parser.add_argument('--remove_specific_information', type=bool, default=False, help='specific information to be removed')
    parser.add_argument('--random_remove_information_percentage', type=float, default=0.0, help='random remove information percentage')
    parser.add_argument('--remove_specific_clients', type=bool, default=False, help='specific clients to be removed')
    ############ Do not change ############
    parser.add_argument('--unlearning_clients', type=bool, default=False, help='unlearning clients')
    parser.add_argument('--unlearning_features', type=bool, default=False, help='unlearning features')
    parser.add_argument('--unlearning_samples', type=bool, default=False, help='unlearning samples')
    parser.add_argument('--unlearning_classes', type=bool, default=False, help='unlearning classes')
    parser.add_argument('--unlearning_specific_information', type=bool, default=False, help='unlearning specific information')
    parser.add_argument('--wth_asynchronous_unlearning', type=bool, default=False, help='whether to use asynchronous training')
    parser.add_argument('--wth_DP', type=bool, default=False, help='whether to use differential privacy')
    parser.add_argument('--regular_h', type=float, default=0.0, help='hyper to regular h')
    parser.add_argument('--apply_R2S', type=bool, default=False, help='wether apply R2S')
    parser.add_argument('--apply_gradient_ascent', type=bool, default=False, help='apply gradient ascent')
    parser.add_argument('--src_file_dir', type=str, default='', help='src file directory')
    parser.add_argument('--trained', type=bool, default=False, help='whether have trained model')
    parser.add_argument('--stream_unlearning', type=bool, default=False, help='stream unlearning')
    ...
    for seed in [97]: # test 5 times 
        ...
        args = load_basic_configs(args.configs, args) # "lr": 0.001, 0.00001 

        # if args.wth_asynchronous_unlearning:
        if args.regular_h:
            args.main_lr = 0.1 * args.main_lr
        ...
        filename = f'unlearning_record.txt'
        args.exp_res_path = args.exp_res_dir + filename
        print(args.exp_res_path)
        print('=============unlearning set====================')
        print('wether use asynchronous unlearning :',args.wth_asynchronous_unlearning)
        if args.unlearning_clients:
            print('unlearning target: clients')
        if args.unlearning_features:
            print('unlearning target: features')
        if args.unlearning_samples:
            print('unlearning target: samples')
        if args.unlearning_classes:
            print('unlearning target: classes')
        if args.unlearning_specific_information:
            print('unlearning target: specific information')
        print('=================================\n')
        ...
        if args.unlearning_features:
            args = load_unlearning_features_configs(args)
            args = load_unlearning_clients_configs(args)
        if args.unlearning_samples:
            args = load_unlearning_samples_configs(args)
            args.clients_to_remove = [args.k-1]
            args.remove_specific_samples = True
        if args.unlearning_clients:
            args = load_unlearning_clients_configs(args)
        if args.unlearning_specific_information:
            args = load_unlearning_information_configs(args)
            args = load_unlearning_clients_configs(args)
        ...
        # load trained models
        args = load_trained_models_configs(args)

        # asynchronous unlearning
        if args.apply_R2S or args.regular_h:
            args.wth_asynchronous_unlearning = False
        if args.wth_asynchronous_unlearning:
            args = load_online_clients_configs(args)
        else:
            args.online_clients = np.arange(args.k)
        for _ in args.clients_to_remove:
            if _ not in args.online_clients:
                args.online_clients = np.append(args.online_clients,_)
        if args.k-1 not in args.online_clients:
            args.online_clients = np.append(args.online_clients,args.k-1)
        args.online_clients = sorted(args.online_clients)
